#include <social_layers/social_layer.h>
...
PLUGINLIB_EXPORT_CLASS(social_layers::SocialLayer, costmap_2d::Layer)
...
namespace social_layers
{
  ...
  bool SocialLayer::isDiscretized()
  {
    return false;
  }

  void SocialLayer::configure(SocialLayerConfig &config, uint32_t level)
  {
    ...
    sub_ = nh.subscribe(topic_, 1, &SocialLayer::callback, this);
  }
  ...
  void SocialLayer::onInitialize()
  {
    ...
    server_ = new dynamic_reconfigure::Server<SocialLayerConfig>(nh);
    f_ = boost::bind(&SocialLayer::configure, this, _1, _2);
    server_->setCallback(f_);

    // sub_ = nh.subscribe("/points_interaction", 1, &SocialLayer::callback, this);

  }

  void SocialLayer::callback(const social_msgs::PointArray& points)
  {
  ...
  void SocialLayer::updateBoundsFromObjects(double* min_x, double* min_y, double* max_x, double* max_y)
  {
    std::list<geometry_msgs::Point>::iterator o_it;
